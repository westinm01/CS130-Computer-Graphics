...
{
    vec3 result;
    vec2 c = Cell_Center(pixel_index);//stores the scalars for u and v in a vec2 c.
    vec3 combine =horizontal_vector*c[0] + vertical_vector*c[1];//combine everything to a vec2 combine using the given formula.
    result=film_position+combine;
    return result;
}
